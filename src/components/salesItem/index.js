...
export default class salesItem extends Component {
  ...
  jump () {
    Taro.navigateTo({
      url: '/pages/salesClueManagement/details/index?originalId=' + this.props.item.originalId
    })
  }
  ...
  render () {
    ...
    let iconImg = null
    if (followStatus==0) {
        iconImg = <Image className='followStatus' src={icon1}></Image>
    } else if (followStatus==1) {
        iconImg = <Image className='followStatus' src={icon2}></Image>
    } else if (followStatus==2) {
        iconImg = <Image className='followStatus' src={icon3}></Image>
    }
        ...
        <View className='listItem' onClick={this.jump}>
            <View className='top'>
                ...
                <View className='right'>
                    <Text className='channelSource'>{this.props.item.channelSource}</Text>
                    {this.props.item.certFlag == 1 && <Image className='logoIcon' src={icon_bbyp}></Image>}
                </View>
            </View>
                ...
                <View className='left'>
                    ...
                    <View className='list'>
                        <View className='label'>
                            <AtIcon prefixClass='iconfont' value='phone' size='16'></AtIcon>
                        </View>
                        <View className='content' onClick={this.makePhoneCall}>{this.props.item.deflossMobile}</View>
                    </View>
                    <View className='list'>
                        <View className='label'>修理厂</View>
                        <View className='content'>
                            {this.props.item.repairFlag == 1 && <Image className='jia' src={jia}></Image>}
